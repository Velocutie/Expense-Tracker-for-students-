...
import { Plus, Minus, Trash2, PiggyBank } from 'lucide-react';
import { Tip } from '@/components/Tip';
import { Modal } from '@/components/Modal';

const MAX_AMOUNT = 10_000_000;

function validateAmount(raw: string, max?: number): string | null {
  ...
  if (max !== undefined && n > max) return `Cannot exceed current saved ₹${max.toLocaleString('en-IN')}.`;
  return null;
}
...
export default function SavedMoneyPage() {
  ...
  const handleAmountChange = (val: string) => {
    ...
    if (amountError) setAmountError(validateAmount(val, formType === 'remove' ? currentSaved : undefined));
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const maxAmt = formType === 'remove' ? currentSaved : undefined;
    const amtErr = validateAmount(form.amount, maxAmt);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => { setFormType('add'); resetForm(); setShowForm(true); }}
            className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-700 transition-all shadow-sm shadow-green-600/20 active:scale-95"
          >
            <Plus size={16} /> Add Saved
          </button>
          <button
            onClick={() => { setFormType('remove'); resetForm(); setShowForm(true); }}
            disabled={currentSaved <= 0}
            className="flex items-center gap-2 px-4 py-2.5 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20 rounded-xl text-sm font-medium hover:bg-red-100 dark:hover:bg-red-500/20 transition-all disabled:opacity-40 active:scale-95"
          >
      ...
      <Modal
        open={showForm}
        onClose={resetForm}
        title={formType === 'add' ? 'Add Saved Money' : 'Remove Saved Money'}
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          <div>
            ...
            <input
              ...
              step="0.01"
              min="0.01"
              max={formType === 'remove' ? currentSaved : undefined}
              value={form.amount}
              onChange={e => handleAmountChange(e.target.value)}
              onBlur={() => setAmountError(validateAmount(form.amount, formType === 'remove' ? currentSaved : undefined))}
              placeholder="0.00"
              className={`w-full px-4 py-2.5 rounded-xl border bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none text-lg transition-colors ${
                amountError ? 'border-red-400 dark:border-red-500' : 'border-gray-200 dark:border-gray-600'
              }`}
              aria-invalid={!!amountError}
              required
            />
            {formType === 'remove' && !amountError && (
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Max: ₹{currentSaved.toLocaleString('en-IN')}</p>
            )}
            {amountError && (
              <p className="mt-1 text-xs text-red-600 dark:text-red-400 font-medium">{amountError}</p>
            )}
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={submitting || !!amountError || !form.amount}
              className={`flex-1 px-4 py-2.5 rounded-xl text-white text-sm font-medium active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                formType === 'add' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
              }`}
            >
          ...
          {submitError && (
            <p className="text-sm text-red-600 dark:text-red-400 text-center bg-red-50 dark:bg-red-500/10 rounded-lg px-3 py-2">{submitError}</p>
          )}
        </form>
      </Modal>
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-700/80">
            {sorted.map(e => (
              <div key={e.id} className="flex items-center gap-3 p-4 hover:bg-gray-50/80 dark:hover:bg-white/[0.03] transition-colors">
                ...
                <button
                  onClick={() => deleteSavedMoneyEntry(e.id)}
